...
def clear_commands_cache() -> None:
    """Drop the module-level memoized commands dict so the next
    load_commands() call re-checks the on-disk cache/source file's mtime.

    Without this, a long-running server process that already called
    load_commands() once will keep serving the stale in-memory dict for the
    rest of the process lifetime — mtime checks in load_commands() never run
    because the early-return above short-circuits before reaching them."""
    global _loaded_commands
    _loaded_commands = None
