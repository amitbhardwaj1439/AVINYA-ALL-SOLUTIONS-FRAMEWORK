...
def write_excel(data: dict[str, pd.DataFrame]) -> None:
    ...
    # Every sheet empty means no command in the commands file matched anything
    # in the upload. openpyxl then refuses to save ("At least one sheet must be
    # visible"), which used to surface to the API as a bare SystemExit and an
    # unhelpful "Parsing failed" — say what actually went wrong instead.
    if all(data.get(key, pd.DataFrame()).empty for _, key in SHEETS):
        raise ValueError(
            "No data could be extracted: none of the commands in the commands "
            "file matched the uploaded file(s). Check that the upload is an "
            "AMOS/hgetc GPL audit log export (each block starting with a "
            "'<node>> hgetc ...' line), not a spreadsheet, a zip, or a log "
            "captured with a different command set."
        )
    ...
    try:
        ...
        # carry the reason out with the exit so callers (the API) can report it
        # rather than guessing — sys.exit(1) alone threw away the diagnosis
        sys.exit(f"Failed to write Excel report: {e}")
